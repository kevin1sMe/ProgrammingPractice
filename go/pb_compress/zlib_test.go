...
//测试使用zlib压缩，对于pb编码后的数据，有多少收益（压缩率能达到多少)
//Usage: go test
...
//Returns:
//     ratio 压缩率 
func ZlibTest(dataSize int) (before int, after int, ratio float32, err error){
...
//zlib
//当sz=250Byte时，zlib压缩有些收益
func Test_CompressRatio_Zlib(t *testing.T) {
    for i:=1; i < 300; i +=5 {
        if b, a, ratio, err := ZlibTest(i) ; err == nil {
            fmt.Printf("pb zlib compress| sz:%d before:%d after:%d ratio:%.2f\n", i, b, a, ratio)
        }
    } 
}
